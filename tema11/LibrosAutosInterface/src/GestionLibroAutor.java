...
public class GestionLibroAutor {
	public static void main(String[] args) {
		...
		Integer opcion, cuantosLibros,posLibro=0,posAutor=0, numComentable=0;

		libros = new Libro[10];
		autores = new Autor[10];

		posAutor=creaAutores(autores,posAutor, comentarios, numComentable);
		posLibro=creaLibros(autores,posLibro,libros, comentarios, 3);
		for (int i=0; i<comentarios.length;i++){
			if (comentarios[i]!=null){
				Leer.mostrarEnPantalla(comentarios[i].comentar());
			}
		}
	...
	private static Integer creaAutor(Autor[] autores, Integer posAutor){
		...
		if (indiceAutor == -1) {
			...
			autor = new Autor(nombreAutor, email, letraSexo,"pacomer");
	...
	public static Integer crearLibros(Integer posLibros, Libro[] libros, Autor[] autores) {
		...
		String titulo, nombreAutor,resena;
		Autor autor;
		Double precio;
		Integer cantidad;
	...
	public static Integer creaAutores(Autor []autores,Integer posAutor, Comentable[] comentarios, Integer numComentable){
		// creamos 3 autores
		autores[0]= new Autor("Rosa Montero", "rmontero@gmail.com", 'f',"paco mermela");
		comentarios[numComentable]=autores[0];
		numComentable++;
		autores[1]= new Autor("Juan Jose Millas", "jjmillas@hotmail.es",'m',"oscar hermoso");
		comentarios[numComentable]=autores[1];
		numComentable++;
		autores[2]= new Autor("Almudena Grandes", "ag@gmail.com",'f',"benito camela");
		comentarios[numComentable]=autores[2];
		numComentable++;
		return 3;
	}// crea 3 autores
	
	public static Integer creaLibros(Autor[] autores, Integer posLibro, Libro[] libros, Comentable[] comentarios, Integer numComentable){
		// creamos 5 libros
		libros[0]= new Libro("Lágrimas en la lluvia", autores[0],19.95 ,100000,"este libro lo escribio oscarin");
		comentarios[numComentable]=libros[0];
		numComentable++;
		libros[1]= new Libro("La ridícula idea de no volver a verte", autores[0], 15., 200000,"este libro antes se llamaba fariña");
		comentarios[numComentable]=libros[1];
		numComentable++;		
		libros[2]= new Libro("Desde la sombra", autores[1], 25., 75000,"el chapo guzman");
		comentarios[numComentable]=libros[2];
		numComentable++;
		libros[3]= new Libro("Malena es un nombre de tango", autores[2], 22., 100000,"este libro es mas malo que la droga");
		comentarios[numComentable]=libros[3];
		numComentable++;		
		libros[4]= new Libro("Las edades de Lulú", autores[2], 30., 200000,"este libro es muy chido");
		comentarios[numComentable]=libros[4];
		numComentable++;		 
		return 5;
	}// crea 5 libros
	...
	public static void modificarLibro(Libro [] libros, String titulo, Autor [] autores){
		...
		if(indiceLibro != -1){
			...
			resena=Leer.pedirCadena("reseña" +libro.getResena()+ " nueva reseña del libro (* no cambiar)");
			libro.setResena(resena);
		}else{
