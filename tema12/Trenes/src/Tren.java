...
public class Tren implements Comparable<Tren> {
	...
	public Tren(String numeroTren, String estacionOrigen, String estacionDestino, String horaSalida) {//Crea los trenes de referencia pero sin lista de vagones
	...
	public void setAsiento(Integer numVagon, Integer asiento) {//reserva un asiento concreto en un vagón determinado 
		boolean hayQueCrearVagon=true;
		for (Vagon v: composicion){
			if (v.getVagon(0).equals(numVagon)){//si encuentra el vagón reserva el asiento
				hayQueCrearVagon=false;
				v.setAsiento(asiento);//reserva el asiento
				break;
			}
		}
		if (hayQueCrearVagon){//si no ha encontrado el vagón lo crea y lo añade
			Vagon vagon=new Vagon(numVagon);//crea el vagón
			vagon.setAsiento(asiento);//reserva el asiento
			composicion.add(vagon);//añade el vagón
		}
	}
